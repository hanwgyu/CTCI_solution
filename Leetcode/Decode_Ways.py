...
class Solution:
    ...
    def numDecodings(self, s: str) -> int:
        ...
        dp_1, dp_2 = (
            (1 if canEncode(s[0]) and canEncode(s[1]) else 0)
            + (1 if canEncode(s[0:2]) else 0),
            1 if canEncode(s[0]) else 0,
        )
        for i in range(2, len(s)):
            dp_1, dp_2 = (
                (dp_1 if canEncode(s[i]) else 0)
                + (dp_2 if canEncode(s[i - 1 : i + 1]) else 0),
                dp_1,
            )
        return dp_1
