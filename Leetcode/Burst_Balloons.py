...
class Solution:
    def maxCoins_2(self, nums: List[int]) -> int:
        def maxCoinsRange(l: int, r: int) -> int:
            ...
            if (l, r) in d:
                return d[(l, r)]
            res = max(nums[l] * nums[r] * nums[i] + maxCoinsRange(l,i) + maxCoinsRange(i,r) for i in range(l+1, r))
            d[(l,r)] = res
            return res
    ...
    def maxCoins_1(self, nums: List[int]) -> int:
        def maxCoinsRange(l: int, r: int) -> int:
            if l+1 == r:
                return 0    
            return max(nums[l] * nums[r] * nums[i] + maxCoinsRange(l,i) + maxCoinsRange(i,r) for i in range(l+1, r))
