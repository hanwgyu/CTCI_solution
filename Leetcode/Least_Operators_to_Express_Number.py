...
class Solution:
        ...
        while y:
            ...
            if p:
                pos, neg = (
                    min(
                        res * self.cost(p) + pos,
                        (res + 1) * self.cost(p) + neg,
                    ),
                    min(
                        (x - res) * self.cost(p) + pos,
                        (x - res - 1) * self.cost(p) + neg,
                    ),
                )
            else:
    ...
    def leastOpsExpressTarget_2(self, x: int, y: int) -> int:
        if x > y:
            return (
                min(self.cost(0) * y, self.cost(0) * (x - y) + self.cost(1))
                - 1
            )
        p, sums = 1, x
        while y > sums:
    ...
    def leastOpsExpressTarget_1(self, x: int, y: int) -> int:
        def minCost(x: int, y: int, p: int) -> int:
            ...
            y, res = divmod(y, x)
            return min(
                minCost(x, y + 1, p + 1) + self.cost(p) * (x - res),
                minCost(x, y, p + 1) + self.cost(p) * res,
            )
